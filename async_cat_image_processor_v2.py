...
from async_logging_config import get_async_logger  # Импортируем асинхронный логгер
    ...
    async def download_images(self, url_generator: AsyncGenerator) -> AsyncGenerator[Tuple[int, np.ndarray, str, str], None]:
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False)) as session:
            async for idx, url, breed in url_generator:
                ...
                result = await self._download_with_retry(session, idx, url, breed, max_retries=3)
